locals = {
  ...
  master_autoscaling_group_ids = ["${aws_autoscaling_group.master-eu-west-1a-masters-mycluster-k8s-local.id}"]
  ...
  node_subnet_ids              = ["${aws_subnet.eu-west-1a-mycluster-k8s-local.id}"]
  ...
  subnet_eu-west-1a_id         = "${aws_subnet.eu-west-1a-mycluster-k8s-local.id}"
...
output "master_autoscaling_group_ids" {
  value = ["${aws_autoscaling_group.master-eu-west-1a-masters-mycluster-k8s-local.id}"]
}
...
output "node_subnet_ids" {
  value = ["${aws_subnet.eu-west-1a-mycluster-k8s-local.id}"]
}
...
resource "aws_autoscaling_attachment" "master-eu-west-1a-masters-mycluster-k8s-local" {
  elb                    = "${aws_elb.api-mycluster-k8s-local.id}"
  autoscaling_group_name = "${aws_autoscaling_group.master-eu-west-1a-masters-mycluster-k8s-local.id}"
}

resource "aws_autoscaling_group" "master-eu-west-1a-masters-mycluster-k8s-local" {
  name                 = "master-eu-west-1a.masters.mycluster.k8s.local"
  launch_configuration = "${aws_launch_configuration.master-eu-west-1a-masters-mycluster-k8s-local.id}"
  max_size             = 1
  min_size             = 1
  vpc_zone_identifier  = ["${aws_subnet.eu-west-1a-mycluster-k8s-local.id}"]
  ...
  tag = {
    key                 = "Name"
    value               = "master-eu-west-1a.masters.mycluster.k8s.local"
    propagate_at_launch = true
  }
...
resource "aws_elb" "api-mycluster-k8s-local" {
  ...
  subnets         = ["${aws_subnet.eu-west-1a-mycluster-k8s-local.id}"]
...
resource "aws_iam_role" "masters-mycluster-k8s-local" {
  name               = "masters.mycluster.k8s.local"
  assume_role_policy = "${file("${path.module}/data/aws_iam_role_masters.mycluster.k8s.local_policy")}"
}

resource "aws_iam_role" "nodes-mycluster-k8s-local" {
  name               = "nodes.mycluster.k8s.local"
  assume_role_policy = "${file("${path.module}/data/aws_iam_role_nodes.mycluster.k8s.local_policy")}"
}

resource "aws_iam_role_policy" "masters-mycluster-k8s-local" {
  name   = "masters.mycluster.k8s.local"
  role   = "${aws_iam_role.masters-mycluster-k8s-local.name}"
  policy = "${file("${path.module}/data/aws_iam_role_policy_masters.mycluster.k8s.local_policy")}"
}

resource "aws_iam_role_policy" "nodes-mycluster-k8s-local" {
  name   = "nodes.mycluster.k8s.local"
  role   = "${aws_iam_role.nodes-mycluster-k8s-local.name}"
  policy = "${file("${path.module}/data/aws_iam_role_policy_nodes.mycluster.k8s.local_policy")}"
}
...
resource "aws_launch_configuration" "master-eu-west-1a-masters-mycluster-k8s-local" {
  ...
  user_data                   = "${file("${path.module}/data/aws_launch_configuration_master-eu-west-1a.masters.mycluster.k8s.local_user_data")}"
...
resource "aws_launch_configuration" "nodes-mycluster-k8s-local" {
  ...
  associate_public_ip_address = true
  user_data                   = "${file("${path.module}/data/aws_launch_configuration_nodes.mycluster.k8s.local_user_data")}"
...
resource "aws_route_table_association" "eu-west-1a-mycluster-k8s-local" {
  subnet_id      = "${aws_subnet.eu-west-1a-mycluster-k8s-local.id}"
  route_table_id = "${aws_route_table.mycluster-k8s-local.id}"
}
...
resource "aws_subnet" "eu-west-1a-mycluster-k8s-local" {
  vpc_id            = "${aws_vpc.mycluster-k8s-local.id}"
  ...
  tags = {
    KubernetesCluster                           = "mycluster.k8s.local"
    Name                                        = "eu-west-1a.mycluster.k8s.local"
    SubnetType                                  = "Public"
    "kubernetes.io/cluster/mycluster.k8s.local" = "owned"
